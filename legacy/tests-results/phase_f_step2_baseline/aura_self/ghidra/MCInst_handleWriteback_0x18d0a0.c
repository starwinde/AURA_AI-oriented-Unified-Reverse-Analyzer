...
void MCInst_handleWriteback(long param_1,long param_2)

{
  ...
  if (bVar1 != 0) {
    ...
    do {
      while( true ) {
        uVar4 = MCOperandInfo_isTiedToOp(lVar3);
        if (((uVar4 & 1) != 0) &&
           (iVar2 = MCOperandInfo_getOperandConstraint
                              (param_2 + (ulong)*(uint *)(param_1 + 8) * 0x10,uVar6 & 0xffffffff,0),
           iVar2 != -1)) break;
LAB_0018d0e8:
        uVar6 = uVar6 + 1;
        lVar3 = lVar3 + 8;
