...
- [x] 2C.2 Ghidra 응답 정규화 — XML byte normalize. *(현재 main 기준: `src/adapter/ghidra/ghidra_xml_normalize.c` 는 BOM/개행/prolog/known-bad prefix 를 바이트 단위로 처리하며 tinyxml2 는 vendored tree 에서 제거됨. XML semantic parse / HIR mapping 은 후속 계층 책임.)*
