...
AuraMcpPathDecision aura_mcp_path_allowed(const std::string& candidate,
                                          const std::string& allowed_roots);
```
...
namespace {
std::vector<std::filesystem::path> splitRoots(const std::string& roots) {
...
bool startsWithPath(const std::filesystem::path& child,
    ...
    auto r = root.begin();
    for (; r != root.end(); ++r, ++c) {
        if (c == child.end() || *c != *r) return false;
    }
    return true;
}
}
...
AuraMcpPathDecision aura_mcp_path_allowed(const std::string& candidate,
                                          const std::string& allowed_roots) {
    AuraMcpPathDecision d;
    std::error_code ec;
    const auto canonical = std::filesystem::weakly_canonical(candidate, ec);
    if (ec) {
        d.reason = "path_unresolved";
        return d;
    }
    d.canonical_path = canonical.string();

    if (!std::filesystem::is_regular_file(canonical, ec)) {
        d.reason = "not_a_regular_file";
        return d;
    }

    for (const auto& rootRaw : splitRoots(allowed_roots)) {
        const auto root = std::filesystem::weakly_canonical(rootRaw, ec);
        if (ec) continue;
...
```cpp
    ...
    if (std::strcmp(name, "aura_info") == 0 ||
        ...
        if (!path.allowed) {
            return aura_mcp_envelope_error(name, name, path.reason.c_str(),
