...
#ifdef AURA_USE_XML_OUT
    /* D-29A Step 0.1: AURA-side ingest uses tinyxml2 instead of
     * implementing PackedDecode. doFormat=false keeps the burst-stream
     * payload compact (no leading spaces / newlines for indentation). */
    XmlEncode encoder(sout, false);
#else
...

{
  ...
  sout.write("\000\000\001\016",4);
#ifdef AURA_USE_XML_OUT
  /* D-29A Step 0.1: matches DecompileAt::rawAction encoder choice. */
  XmlEncode encoder(sout, false);
#else
  PackedEncode encoder(sout);
#endif
...
#ifdef AURA_ARGV_MODE
  /* D-29A: scan argv for AURA flags. If found, dispatch to argv mode and
   * skip the legacy GhidraCapability::readCommand subprocess loop. The
   * legacy path is preserved when no AURA flag is present (Ghidra Java
   * client compatibility). Frozen contract — see plan §"argv 인자 사양
   * Fixation Amendment". */
  for (int i = 1; i < argc; ++i) {
    if (std::strcmp(argv[i], "--decompile") == 0 ||
        std::strncmp(argv[i], "--decompile=", 12) == 0 ||
        std::strcmp(argv[i], "--help") == 0 ||
        std::strcmp(argv[i], "--version") == 0) {
      AttributeId::initialize();
      ElementId::initialize();
      CapabilityPoint::initializeAll();
      int rc = aura_argv_mode_entry(argc, argv);
